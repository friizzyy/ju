...
const PricingCard: React.FC<{ plan: PricingPlan; className?: string }> = ({ plan, className = '' }) => (
  <motion.div
    variants={{
      hidden: { opacity: 0, y: 28, scale: 0.97 },
      visible: { opacity: 1, y: 0, scale: 1, transition: { duration: 0.55, ease: [0.22, 1, 0.36, 1] } }
    }}
    className={`relative flex flex-col rounded-2xl border transition-all duration-700 group cursor-default overflow-visible ${plan.featured ? 'lg:-translate-y-4' : ''} ${className}`}
    style={{
...
const PricingCards: React.FC<{ plans: PricingPlan[]; columns?: 3 | 4 }> = ({ plans, columns = 3 }) => {
  return (
    <>
      ...
      {/* Desktop: grid */}
      <motion.div
        className={`hidden sm:grid sm:grid-cols-2 ${columns === 4 ? 'lg:grid-cols-4' : 'lg:grid-cols-3'} gap-4 lg:gap-5 items-end pt-8`}
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, margin: "-60px" }}
        variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.1, delayChildren: 0.1 } } }}
